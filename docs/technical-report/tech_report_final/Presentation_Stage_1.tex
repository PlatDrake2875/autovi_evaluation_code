...
\begin{block}{Category-Based Client Partitioning}
  \small
  Each client represents a different production line for one component type
\end{block}
...
\begin{table}
  ...
  \begin{tabular}{llcc}
    ...
    \bottomrule
  \end{tabular}
  \caption{\tiny Stage 1 Client Configuration}
\end{table}
...
\begin{block}{Stage 1 Approach}
  \tiny
  \begin{itemize}
    \item Each client trains independently on assigned category
...
\begin{block}{From Image to Anomaly Map}
  ...
  \begin{enumerate}
    ...
    \item Apply neighborhood averaging (3×3 smoothing)
    \item Reshape to patch vectors
    \item Query FAISS index for distances
    \item Upsample scores to original resolution
    \item Compute image-level score (max of map)
  \end{enumerate}
\end{block}
...
\begin{block}{Evaluation Metrics}
  \tiny
  \begin{itemize}
    \item \textbf{AUC-sPRO}: Pixel-level localization (FPR: 0.01, 0.05, 0.1, 0.3)
    \item \textbf{AUC-ROC}: Image-level classification
  \end{itemize}
\end{block}

\vspace{0.1cm}

\textit{\tiny Feature extractor (WideResNet-50-2) frozen; no network weight training}
...
\column{0.5\textwidth}
  \begin{block}{Privacy Enhancement}
    ...
    \begin{itemize}
      ...
      \item Calibrated noise on coresets
...
\column{0.5\textwidth}
  \begin{block}{Fairness: Cross-Category Equity}
    ...
    \begin{itemize}
      ...
      \item Address non-IID heterogeneity
...
\begin{block}{Combined Goal}
  \tiny
  Transform Stage 1 into a \textbf{production-ready system} where:
  \begin{itemize}
    \item Data privacy is paramount
    \item Regulatory compliance required
    \item Cross-facility fairness essential
  \end{itemize}
\end{block}
...
\begin{block}{Stage 1 Achievements}
  ...
  \begin{enumerate}
    ...
    \item PatchCore implementation (feature extraction, memory bank, FAISS)
...
\begin{block}{Next Steps}
  ...
  \begin{itemize}
    ...
    \item Analyze per-category performance metrics
...
\begin{block}{Stage 2 Vision}
  \tiny
  Enhance Stage 1 with privacy-preserving and fairness mechanisms to create a trustworthy federated learning system for industrial inspection.
\end{block}
